...
export default function MasterScheduleTableView({
  ...
  const renderTable = () => (
    <table ref={tableRef} className="min-w-full border-separate border-spacing-0 text-sm break-words">
      <thead>
        <tr>
          <th className="sticky left-0 top-0 z-50 h-10 bg-surface px-4 text-left text-xs font-semibold uppercase tracking-wide text-muted">
            Должность
          </th>
          {dates.map((date) => {
            ...
            return (
              <th
                key={`weekday-${date}`}
                className={`sticky top-0 z-30 h-10 border-b border-subtle px-3 text-center text-xs font-semibold uppercase tracking-wide text-muted ${
                  isFriday || isSaturday ? "bg-orange-50" : "bg-surface"
                } ${isMonday ? "border-l-2 border-subtle" : "border-l border-subtle"}`}
              >
                {weekday}
              </th>
            );
        ...
        <tr>
          <th className="sticky left-0 top-10 z-40 h-10 bg-surface px-4 text-left text-xs font-medium text-icon">
            Строки
          </th>
          {dates.map((date) => {
            ...
            return (
              <th
                key={`day-${date}`}
                className={`sticky top-10 z-20 h-10 border-b border-subtle px-3 text-center text-xs font-medium text-muted ${
                  isFriday || isSaturday ? "bg-orange-50" : "bg-surface"
                } ${isMonday ? "border-l-2 border-subtle" : "border-l border-subtle"}`}
              >
                {day}.{month}
      ...
      <tbody>
          ...
          return (
            <React.Fragment key={group.positionId}>
              {group.rows.map((row, index) => {
                ...
                return (
                  <tr key={row.id} className="border-b border-subtle">
                    <td className="sticky left-0 z-10 border-r border-subtle bg-surface px-4 py-2 break-words">
                      <div className="flex items-center justify-between gap-2">
                        <div className="min-w-0">
                          ...
                          <div className="text-xs text-icon">
                        ...
                        <div className="flex items-center gap-1">
                          <button
                            type="button"
                            className="rounded-xl border border-subtle bg-surface p-1.5 text-icon hover:bg-app"
                      ...
                      {isLast && (
                        <button
                          type="button"
                          className="mt-2 inline-flex items-center gap-2 rounded-2xl border border-subtle bg-surface px-3 py-1 text-xs text-muted hover:bg-app"
                          onClick={() => onAddRow(group.positionId)}
                        >
                          <Icon icon={Plus} size="xs" className="text-icon" /> Добавить строку
                        </button>
                      )}
                    </td>
                    {dates.map((date, colIndex) => {
                      ...
                      return (
                        <td
                          key={cellId}
                          className={`border-l border-subtle px-2 py-1 ${
                            isFriday || isSaturday ? "bg-orange-50" : "bg-surface"
                          } ${isMonday ? "border-l-2 border-subtle" : ""}`}
                        >
                          <input
                            className={`w-full rounded-lg border px-2 py-1 text-center text-sm outline-none focus:ring-2 ${
                              hasError
                                ? "border-red-400 focus:ring-red-200"
                                : "border-subtle ring-default"
                            }`}
                            value={cell?.valueRaw ?? ""}
              ...
              <tr className="border-b border-subtle bg-app">
                <td className="sticky left-0 z-10 border-r border-subtle bg-app px-4 py-2 text-sm font-semibold text-default">
                  Итого: {group.positionName}
                </td>
                {groupTotals.map((value, idx) => (
                  <td
                    key={`group-${group.positionId}-${dates[idx]}`}
                    className="border-l border-subtle px-2 py-1 text-center text-sm font-medium text-muted"
                  >
                    {formatNumber(value)}
                  </td>
                ))}
              ...
              <tr className="border-b border-subtle bg-app">
                <td className="sticky left-0 z-10 border-r border-subtle bg-app px-4 py-1 text-xs text-muted">
                  Итого по должности: {formatNumber(groupRowTotal)}
                </td>
                {dates.map((date) => (
                  <td
                    key={`group-spacer-${group.positionId}-${date}`}
                    className="border-l border-subtle bg-app px-2 py-1"
                  />
                ))}
              </tr>
            </React.Fragment>
        ...
        <tr className="border-t border-subtle bg-app">
          <td className="sticky left-0 z-10 bg-app px-4 py-2 text-sm font-semibold text-strong">
            Итог по дням
          </td>
          {columnTotals.map((value, index) => (
            <td
              key={`total-${dates[index]}`}
              className="border-l border-subtle px-2 py-1 text-center text-sm font-medium text-default"
            >
              {formatNumber(value)}
            </td>
          ))}
  ...
  return (
    <div
      ref={containerRef}
      className={`${containerClassName} rounded-3xl border border-subtle bg-surface shadow-[var(--staffly-shadow)]`}
    >
      {overviewMode ? (
      ...
      <div className="flex flex-wrap justify-between gap-3 border-t border-subtle bg-surface px-4 py-3 text-sm">
        <div className="text-muted">Общий итог ФОТ</div>
        <div className="font-semibold text-strong">{formatNumber(totalPayroll)}</div>
      </div>
    </div>
  );
}
