...
import type { ChecklistRequest } from "../api";
...
type ChecklistDialogProps = {
  ...
  initialData?: { name: string; content: string; positionIds: number[] };
  submitting: boolean;
  error?: string | null;
  onClose: () => void;
...
const ChecklistDialog: React.FC<ChecklistDialogProps> = ({
  ...
  const [fields, setFields] = React.useState<PositionField[]>([]);
  ...
  React.useEffect(() => {
    ...
    setLocalError(null);
  }, [open, initialData?.name, initialData?.content, initialData?.positionIds]);
  ...
  const handleChange = React.useCallback((id: string, value: string) => {
    setFields((prev) => prev.map((field) => (field.id === id ? { ...field, value: value ? Number(value) : "" } : field)));
  }, []);
