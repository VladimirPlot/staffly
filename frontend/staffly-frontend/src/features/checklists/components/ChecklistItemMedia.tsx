...
import type { ChecklistDto, ChecklistItemDto, ChecklistPhotoMode } from "../api";
...
export default function ChecklistItemMedia({
  ...
  const canAttachCompletionPhoto = photoMode !== "NONE";
  const showCompletionPhoto = canAttachCompletionPhoto || hasCompletionPhoto;
  const missingRequiredPhoto = photoMode === "REQUIRED" && !hasCompletionPhoto;

  if (!hasExamplePhoto && !showCompletionPhoto) {
    return null;
  }

  return (
    <div
      className={`border-subtle mt-3 grid gap-2 border-t pt-3 md:gap-3 ${
        hasExamplePhoto && showCompletionPhoto ? "grid-cols-2" : "grid-cols-1"
      }`}
    >
      {hasExamplePhoto && (
              ...
              >
                {hasCompletionPhoto
                  ...
                  : photoMode === "REQUIRED"
                    ...
                    : "Можно приложить по желанию"}
              </div>
              {!item.done && canAttachCompletionPhoto && (
