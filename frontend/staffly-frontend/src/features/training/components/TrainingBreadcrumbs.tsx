...
import { parseTrainingObjectId, trainingFolderDropId } from "../trainingFolderDnd";
import { trainingRoutes } from "../utils/trainingRoutes";

function TrainingBreadcrumbFolder({
  folder,
  isCurrent,
  isDragActive,
  disabledDrop,
  ...
  const drop = useDroppable({ id: trainingFolderDropId(folder.id), disabled: disabledDrop });
...
export default function TrainingBreadcrumbs({
  ...
  return (
    ...
    >
      ...
      <Link
        to={trainingRoutes.landing}
        className="text-default inline-flex h-8 shrink-0 items-center rounded-lg px-1.5 font-medium transition hover:bg-[var(--staffly-control-hover)] focus:ring-2 focus:ring-[var(--staffly-ring)] focus:outline-none focus:ring-inset"
      >
        Тренинг
      </Link>
      <Icon icon={ChevronRight} size="xs" decorative className="text-icon shrink-0 opacity-55" />
