...
export default function QuestionEditorModal({ open, restaurantId, folderId, question, onClose, onSaved }: Props) {
  ...
  const [options, setOptions] = useState<Option[]>([{ text: "", correct: true }, { text: "", correct: false }]);
  const [pairs, setPairs] = useState<Pair[]>([{ leftText: "", rightText: "" }, { leftText: "", rightText: "" }]);
  ...
  useEffect(() => {
    ...
    setOptions(question?.options?.length ? question.options.map((o) => ({ text: o.text, correct: o.correct })) : [{ text: "", correct: true }, { text: "", correct: false }]);
    setPairs(question?.matchPairs?.length ? question.matchPairs.map((p) => ({ leftText: p.leftText, rightText: p.rightText })) : [{ leftText: "", rightText: "" }, { leftText: "", rightText: "" }]);
  ...
  const addBlank = () => {
    ...
    setBlanks((prev) => [...prev, { index: next, options: [{ text: "", correct: true, sortOrder: 0 }, { text: "", correct: false, sortOrder: 1 }] }]);
  };

  const removeBlank = (index: number) => {
    const reindexed = blanks.filter((b) => b.index !== index).map((b, i) => ({ ...b, index: i + 1 }));
  ...
  const move = <T,>(arr: T[], i: number, dir: -1 | 1): T[] => {
    const n = i + dir; if (n < 0 || n >= arr.length) return arr;
    const copy = [...arr]; [copy[i], copy[n]] = [copy[n], copy[i]]; return copy;
  };
  ...
  const submit = async () => {
    ...
    try {
      const payload = {
        ...
        options: type === "MATCH" || type === "FILL_SELECT" ? [] : (type === "TRUE_FALSE" ? [{ text: "Правда", correct: options[0]?.correct ?? true, sortOrder: 0 }, { text: "Ложь", correct: options[1]?.correct ?? false, sortOrder: 1 }] : options.map((o, i) => ({ ...o, sortOrder: i }))),
        matchPairs: type === "MATCH" ? pairs.map((p, i) => ({ ...p, sortOrder: i })) : [],
        blanks: type === "FILL_SELECT" ? blanks.map((b) => ({ ...b, options: b.options.map((o, i) => ({ ...o, sortOrder: i })) })) : [],
      };
  ...
  return <Modal open={open} onClose={onClose} title={question ? "Редактирование вопроса" : (step === "usage" ? "Вид вопроса" : step === "type" ? "Тип вопроса" : "Новый вопрос")}
    ...
    {step === "editor" && <div className="space-y-3"><Input label="Название" value={title} onChange={(e) => setTitle(e.target.value)} required /><Textarea label="Формулировка" value={prompt} onChange={(e) => setPrompt(e.target.value)} required /><Textarea label="Пояснение" hint="Показывается пользователю после ответа и объясняет правильный вариант." value={explanation} onChange={(e) => setExplanation(e.target.value)} />
      ...
      {type === "FILL_SELECT" && <div className="space-y-3"><Button variant="outline" onClick={addBlank}><Plus className="h-4 w-4" /> Добавить пропуск</Button>{blanks.map((blank) => <div key={blank.index} className="rounded-2xl border border-subtle p-3 space-y-2"><div className="flex items-center justify-between"><div className="font-medium">Пропуск {blank.index}</div><IconButton onClick={() => removeBlank(blank.index)}><Trash2 className="h-4 w-4" /></IconButton></div>{blank.options.map((o, i) => <div key={i} className="flex items-center gap-2"><input type="radio" name={`blank-${blank.index}`} checked={o.correct} onChange={() => setBlanks((prev) => prev.map((b) => b.index === blank.index ? { ...b, options: b.options.map((x, idx) => ({ ...x, correct: idx === i })) } : b))} /><Input label="Вариант" className="flex-1" value={o.text} onChange={(e) => setBlanks((prev) => prev.map((b) => b.index === blank.index ? { ...b, options: b.options.map((x, idx) => idx === i ? { ...x, text: e.target.value } : x) } : b))} error={duplicates.has(`blank-${blank.index}-${i}`) ? "Дубликат" : undefined} /><IconButton onClick={() => setBlanks((prev) => prev.map((b) => b.index === blank.index ? { ...b, options: b.options.filter((_, idx) => idx !== i) } : b))}><Trash2 className="h-4 w-4" /></IconButton></div>)}<Button variant="outline" onClick={() => setBlanks((prev) => prev.map((b) => b.index === blank.index ? { ...b, options: [...b.options, { text: "", correct: false, sortOrder: b.options.length }] } : b))}>Добавить вариант</Button></div>)}</div>}
