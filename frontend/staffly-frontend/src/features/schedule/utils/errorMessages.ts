...
export function getFriendlyScheduleErrorMessage(error: unknown, fallback: string): string {
  ...
  if (maybeError.response?.data?.error === "SCHEDULE_BUILD_TEMPLATE_VERSION_CONFLICT") {
    return "Шаблон был изменён другим пользователем. Обновите данные и повторите изменения.";
  }
  ...
  if (maybeError.response?.data?.error === "SCHEDULE_PREFERENCE_REVISION_CONFLICT") {
    return "Пожелания были изменены в другой сессии. Обновите данные и повторите изменения.";
  }
