...
class CertificationAnalyticsLifecycleCoordinator {
    ...
    public void normalizeCurrentExams(Long restaurantId, Collection<Long> examIds, Instant now) {
        ...
        var currentAssignments = assignments.findCurrentCertificationObligationsForAnalyticsUpdate(restaurantId, requestedIds);
        var unfinished = attempts.findUnfinishedForCurrentObligationsAnalyticsScope(restaurantId, requestedIds);
        ...
        Map<Long, List<TrainingExamAttempt>> finishedByAssignment = attempts
                .findFinishedForCurrentObligationsAnalyticsScope(restaurantId, requestedIds).stream()
                .collect(Collectors.groupingBy(a -> a.getAssignment().getId()));

        for (var assignment : currentAssignments) {
            validateCurrentObligationIdentity(assignment);
    ...
    private void validateCurrentObligationIdentity(ru.staffly.training.model.TrainingExamAssignment assignment) {
        ...
        if (assignment.getStatus() == TrainingExamAssignmentStatus.ARCHIVED
            ...
            throw new IllegalStateException("Current Certification obligation identity is inconsistent: assignmentId="
                    + assignment.getId());
        }
    }
